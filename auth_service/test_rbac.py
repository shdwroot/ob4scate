...
from .rbac_engine import RBACEngine, RBACResource, User
...
class TestRBACEngine:
    ...
    def super_admin_user(self):
        """Super admin user"""
        return User(user_id="admin1", username="admin", roles=["super_admin"], tenant_id="tenant1")
    ...
    def data_scientist_user(self):
        """Data scientist user"""
        return User(
            user_id="ds1", username="data_scientist", roles=["data_scientist"], tenant_id="tenant1"
        )

    @pytest.fixture
    def operator_user(self):
        """Operator user"""
        return User(user_id="op1", username="operator", roles=["operator"], tenant_id="tenant1")

    @pytest.fixture
    def regular_user(self):
        """Regular user"""
        return User(user_id="user1", username="user", roles=["user"], tenant_id="tenant1")

    @pytest.fixture
    def policy_resource(self):
        """Policy resource"""
        return RBACResource(resource_type="policy", resource_id="policy1", tenant_id="tenant1")
    ...
    def test_security_admin_permissions(self, rbac_engine, security_admin_user, policy_resource):
        ...
        assert not rbac_engine.check_permission(
            security_admin_user, "model:fine_tune", policy_resource
        )
    ...
    def test_data_scientist_permissions(self, rbac_engine, data_scientist_user, policy_resource):
        ...
        assert not rbac_engine.check_permission(
            data_scientist_user, "policy:write", policy_resource
        )
    ...
    def test_tenant_isolation(self, rbac_engine):
        ...
        resource_tenant1 = RBACResource(
            resource_type="policy", resource_id="policy1", tenant_id="tenant1"
        )

        resource_tenant2 = RBACResource(
            resource_type="policy", resource_id="policy1", tenant_id="tenant2"
        )
        ...
        assert not rbac_engine.check_permission(user1, "gateway:use", resource_tenant2)

    def test_direct_permissions_and_unknown_values(self, rbac_engine):
        user = User(
            user_id="u1",
            username="user1",
            roles=[],
        ...
        resource = RBACResource("policy", "one", tenant_id="tenant1")
        assert rbac_engine.check_permission(user, "policy:read", resource)
        assert not rbac_engine.check_permission(user, "not:a:permission", resource)
        with pytest.raises(ValueError, match="Unknown permission"):
            rbac_engine.add_user_permission(user, "not:a:permission")
        with pytest.raises(ValueError, match="Unknown role"):
            rbac_engine.add_user_role(user, "not-a-role")
...
class TestRBACMiddleware:
    ...
    def mock_user(self):
        ...
        return User(
            user_id="test_user", username="testuser", roles=["data_scientist"], tenant_id="tenant1"
        )
    ...
    def test_require_any_permission_decorator(self, rbac_middleware, mock_user):
        ...
        with patch.object(
            rbac_middleware.rbac_engine, "check_permission", side_effect=mock_check_permission
        ):
            result = asyncio.run(test_endpoint(current_user=mock_user))
            assert result["message"] == "success"
