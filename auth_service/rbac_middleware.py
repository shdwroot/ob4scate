...
from .rbac_engine import Permission, RBACResource, User, rbac_engine
...
class RBACMiddleware:
    ...
    def require_permission(self, permission: str, resource_type: str | None = None):
    ...
    def require_any_permission(self, permissions: list[str], resource_type: str | None = None):
            ...
            async def wrapper(*args, **kwargs):
                ...
                if not has_permission:
                    raise HTTPException(
                        status_code=status.HTTP_403_FORBIDDEN,
                        detail=(
                            f"Insufficient permissions. Required one of: {', '.join(permissions)}"
                        ),
                    )
    ...
    def require_all_permissions(self, permissions: list[str], resource_type: str | None = None):
...
def require_any_permission(permissions: list[str], resource_type: str | None = None):
    """Decorator to require any of the specified permissions"""
    return rbac_middleware.require_any_permission(permissions, resource_type)


def require_all_permissions(permissions: list[str], resource_type: str | None = None):
    """Decorator to require all of the specified permissions"""
